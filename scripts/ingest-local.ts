import * as fs from "fs";
import * as path from "path";
import { createClient } from "@supabase/supabase-js";
import * as dotenv from "dotenv";

// Charger .env.local
dotenv.config({ path: ".env.local" });
dotenv.config({ path: ".env" });

const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL!;
const SUPABASE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY!;
const HF_KEY       = process.env.HUGGINGFACE_API_KEY!;

if (!SUPABASE_URL || !SUPABASE_KEY || !HF_KEY) {
  console.error("❌ Variables manquantes dans .env.local :");
  console.error("   NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, HUGGINGFACE_API_KEY");
  process.exit(1);
}

const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);

const GEO_BOOST: Record<string, number> = {
  "malaria": 1.9, "typhoid fever": 1.7, "cholera": 1.5,
  "dengue fever": 1.4, "meningitis": 1.3, "yellow fever": 1.3,
  "tuberculosis": 1.3, "sickle-cell anemia": 1.4,
  "lassa fever": 1.2, "hepatitis": 1.2, "pneumonia": 1.2,
};

async function embed(text: string): Promise<number[]> {
  for (let attempt = 0; attempt < 4; attempt++) {
    try {
      const res = await fetch(
        "https://api-inference.huggingface.co/pipeline/feature-extraction/sentence-transformers/all-MiniLM-L6-v2",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${HF_KEY}`,
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
            inputs: text.slice(0, 400),
            options: { wait_for_model: true },
          }),
        }
      );

      if (res.status === 503) {
        // Modèle en cours de chargement — attendre
        console.log("   ⏳ HF model loading, waiting 10s...");
        await new Promise(r => setTimeout(r, 10000));
        continue;
      }

      if (!res.ok) throw new Error(`HF ${res.status}: ${await res.text()}`);

      const raw = await res.json() as number[] | number[][];
      return Array.isArray(raw[0]) ? (raw[0] as number[]) : (raw as number[]);

    } catch (err) {
      if (attempt === 3) throw err;
      await new Promise(r => setTimeout(r, 2000 * (attempt + 1)));
    }
  }
  throw new Error("Embed failed");
}

function parseCSV(content: string): Record<string, string>[] {
  const lines = content.trim().split("\n");
  const headers = lines[0]
    .split(",")
    .map(h => h.trim().replace(/"/g, "").replace(/\r/g, ""));
  return lines.slice(1).map(line => {
    const values = line.split(",")
      .map(v => v.trim().replace(/"/g, "").replace(/\r/g, ""));
    const row: Record<string, string> = {};
    headers.forEach((h, i) => { row[h] = values[i] || "0"; });
    return row;
  });
}

async function main() {
  console.log("🚀 PulseAI — Ingestion locale (HuggingFace API)\n");

  // Vérifier si déjà fait
  const { count } = await supabase
    .from("disease_embeddings")
    .select("*", { count: "exact", head: true });

  if (count && count > 0) {
    console.log(`✅ Déjà ingéré : ${count} maladies dans Supabase.`);
    return;
  }

  const csvPath = path.join(process.cwd(), "public", "data", "data_symptom.csv");
  if (!fs.existsSync(csvPath)) {
    console.error("❌ CSV introuvable :", csvPath);
    process.exit(1);
  }

  const rows = parseCSV(fs.readFileSync(csvPath, "utf-8"));

  // Dédoublonner
  const diseaseMap = new Map<string, Set<string>>();
  for (const row of rows) {
    const disease = row["diseases"]?.trim();
    if (!disease) continue;
    if (!diseaseMap.has(disease)) diseaseMap.set(disease, new Set());
    Object.entries(row).forEach(([key, val]) => {
      if (key !== "diseases" && val === "1") {
        diseaseMap.get(disease)!.add(key.replace(/_/g, " "));
      }
    });
  }

  const diseases = Array.from(diseaseMap.entries());
  console.log(`🔬 ${diseases.length} maladies uniques\n`);

  let success = 0;
  let errors  = 0;

  for (const [disease, symptomsSet] of diseases) {
    try {
      const symptomsText = Array.from(symptomsSet).join(", ");
      const richText     = `Patient with ${disease}: ${symptomsText}. West Africa.`;
      const geoBoost     = GEO_BOOST[disease.toLowerCase()] || 1.0;

      // Un seul embedding (pas double) pour économiser les appels HF
      const embedding = await embed(symptomsText);

      const { error } = await supabase.from("disease_embeddings").insert({
        disease_name:  disease,
        symptoms_text: symptomsText,
        rich_text:     richText,
        symptom_list:  Array.from(symptomsSet),
        geo_boost:     geoBoost,
        embedding,
      });

      if (error) throw new Error(error.message);

      success++;
      const pct  = Math.floor((success / diseases.length) * 100);
      const bar  = "█".repeat(Math.floor(pct / 5)).padEnd(20);
      process.stdout.write(
        `\r  [${bar}] ${pct}% — ${success}/${diseases.length} ${disease.slice(0, 25).padEnd(25)}`
      );

      // Rate limit HF free tier : 1 req/sec
      await new Promise(r => setTimeout(r, 1100));

    } catch (err: any) {
      errors++;
      console.error(`\n  ✗ ${disease}: ${err.message}`);
    }
  }

  console.log(`\n\n✅ Terminé : ${success} ok, ${errors} erreurs`);

  // Vérification finale
  const { count: finalCount } = await supabase
    .from("disease_embeddings")
    .select("*", { count: "exact", head: true });
  console.log(`📊 Supabase disease_embeddings : ${finalCount} entrées`);
}

main().catch(err => {
  console.error("\n💥 Fatal:", err.message);
  process.exit(1);
});